...
from .Relation_extraciton_dp import RelationExtraciton
...
class AcquireAllTargetInfo:
    ...
    def get_text_triple(self, prop_name):
        ...
        for i in range(0, length):
            ...
            file = open(os.path.join(text_path, n_path), 'r', encoding='utf-8')
            data = file.read()
            ...
            for a in all_structureinfo:
                ...
                for n in i:
                    for w, v in n.items():
                        if w == 'value':
                            b += 1
            out_path = os.path.join(self.out_path, str(prop_name)+'.xlsx')
