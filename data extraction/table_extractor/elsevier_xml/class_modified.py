        ...
        for num, (doi, f) in enumerate(zip(dois, files)):
            ...
            if 'html' in f:
                ...
                for table, row_ind, col_ind, orient, table_ref in zip(tables, row_inds, col_inds, orients, table_refs):
                    tab.append(self.construct_table_object(doi, table, row_ind, col_ind))
                ...
                try:
                    tables, captions = self.get_xml_tables(doi, f)
                    cols, rows, col_inds, row_inds = self.get_headers(tables)
                    pred_cols, pred_rows = self.classify_table_headers(cols, rows)
                    orients = []
                    composition_flags = []
                ...
                if len(footer) > 0:
                    for f in footer:
                        ...
                        if dds is None or dts is None:
                            ...
                            footer_dict[dts.text.strip()] = unidecode.unidecode(html.unescape(dds.text)).strip()
                else:
                    return None
            elif '10.1039' in doi:
                ...
                if footer is None:
                    ...
                    if len(dds) != len(dts):
                        ...
                        for d, t in zip(dds, dts):
                            footer_dict[t.text.strip()] = unidecode.unidecode(html.unescape(d.text)).strip()
            elif '10.1002' in doi:
                up = table.parent
                ...
                if next is None:
                    ...
                    if footer is None:
                        ...
                        if len(dts) != len(dds):
                            ...
                            for d, t in zip(dds, dts):
                                footer_dict[t.strip()] = unidecode.unidecode(html.unescape(d)).strip()
            elif '10.1007' in doi:
                up = table.parent
                next = up.next_sibling
                if next is None:
                ...
                if footer is not None:
                    ...
                    if len(dts) != len(dds):
                        ...
                        if len(dds) == 1 and len(dts) > 1:
                            ...
                            for d, t in zip(dds, dts):
                                footer_dict[t.strip()] = unidecode.unidecode(html.unescape(d)).strip().replace(
                                    '\n',
                                    ' ')
                            ...
                            for d, t in zip(dds, dts):
                                footer_dict[t.text.strip()] = unidecode.unidecode(
                                    html.unescape(d.text)).strip().replace('\n', ' ')
                    else:
                        for d, t in zip(dds, dts):
                            footer_dict[t.text.strip()] = unidecode.unidecode(
                                html.unescape(d.text)).strip().replace('\n', ' ')
...
def get_extraction_outcome(xml_path, save_path, config_path):
    ...
    for file_i in range(len(os.listdir(xml_path))):
        ...
        try:
            ...
            captions = None
        if tables:
            cols, rows, col_inds, row_inds = table_extractor_m.get_headers(tables)
            tab = []
            for table, row_ind, col_ind in zip(tables, row_inds, col_inds):
