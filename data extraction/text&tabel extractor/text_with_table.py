...
from output_modified_triple import TableExtraction, GetTargetInfo
import copy
...
class AcquireTargetInfo:
    ...
    def get_text_triple(self, prop_name):
        ...
        for i in range(0, length):
            ...
            for index, sent in target_sents.items():
                ...
                ree = RelationExtraciton(
                    prop_name, filter_data, sub_order, sub_id, object_list, self.c_path, abbre_pairs)
    ...
    def run(self):
        ...
        for prop_name in self.prop_list:
            ...
            te = TableExtraction(self.excels_path, self.c_path, prop_name=target_property)
            ...
            gather_outcome = self.gather_tableinfo_textinfo(
                all_txt_info, info_all, prop_name, prop_pattern, unit_pattern_text)
            gather = 0
            for q in gather_outcome:
                k = tuple(q.keys())[0]
                i = q[k]
